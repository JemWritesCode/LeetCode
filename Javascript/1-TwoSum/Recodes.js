...
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case the needed diff is at the end of the array so we traverse the entire array looking for it.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case the needed diff is at the end of the array so our map becomes a copy of the nums array.
...
// #region 39 -- July 12, 2026
/**
 * @param {number[]} nums
 * @param {number} target
 * @return {number[]}
 */
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case we much visit all elements to find the needed diff.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case we must store all the numbers in the map as we traverse through the nums array looking for the diff.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
...
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst cae the needed diff is at the end of the array and we traverse the entire array looking for it.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the array. We end up storing a copy into map.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
...
// Time Complexity: O(n) where n is equal to the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the array so we search the entire array looking for it.
// Space Complecity: O(n) where n is equal to the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the array so we copy the entire array into the map.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
...
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case the needed diff is the last element of the array so we search the entire array looking for it.
...
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is the last element so we traverse the entire array looking for it.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is the last element so the Hashmap becomes a copy of the nums array.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
...
// Time Complexity: O(n) in the worst case the needed diff is at the end of the nums array and we search the whole array looking for it.
// Space Complexity: O(n) in the worst case the needed diff is at the end of the array. so we store every element of the nums array into the map.
...
//Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the nums array and we copy every element from the nums array into the map.
var twoSum = function (nums, target) {
  const map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
...
// Asymptotic Notation
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the nums array and we traverse every element of it looking for it.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the nums array so we copy every element into the map as we look for it.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
...
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed number is at the end of the nums array and we traverse the entire array looking for it.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed number is at the end of the nums array and we copy every element of the nums array into the hashmap.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
 ...
 */
// Time Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the nums array and we traverse the entire array looking for it.
// Space Complexity: O(n) where n is the number of elements in the nums array. In the worst case scenario the needed diff is at the end of the nums array and we add every element to the map as we look for it.
var twoSum = function (nums, target) {
  let map = new Map();
  for (let i = 0; i < nums.length; i++) {
    let diff = target - nums[i];
