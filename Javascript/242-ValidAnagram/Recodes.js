...
// Time Complexity: O(n) -- where n is the number of characters in either string. We must traverse the entire string to count the frequency of each letter of the alphabet.
// Space Complexity: O(1) - We only create offset and frequencyCounter which are both fixed size no matter the size of the strings passed in.
...
// #region 35 -- July 10, 2026
/**
 * @param {string} s
 * @param {string} t
 * @return {boolean}
 */
// Time Complexity: O(n) where n is the number of characters in the s string. We have to count the frequency of every character in the string by traversing through it.
// Space Complexity: O(1) - We traverse the strings in place. We only need the frequency array to count the characters and that's set at 26 for the alphabet.
var isAnagram = function (s, t) {
  if (s.length != t.length) return false;
...
// Time Complexity: O(n) where n is the number of characters in the s string. We must visit each character in the s string to count the frequency of each character.
// Space Complexity: O(1) no matter the size of the arrays passed in we just need the one frequency array of 26 characters of the alphabet. We traverse the arrays in place.
var isAnagram = function (s, t) {
  if (s.length !== t.length) return false;
  let freqCounter = new Array(26).fill(0);
  for (let i = 0; i < s.length; i++) {
    freqCounter[s.charCodeAt(i) - "a".charCodeAt(0)]++;
    freqCounter[t.charCodeAt(i) - "a".charCodeAt(0)]--;
  }
  return freqCounter.every((count) => count === 0);
};
...
// #region 29 -- July 7 2025
/**
 * @param {string} s
 * @param {string} t
 * @return {boolean}
 */
// Frequency Counter - We check to see if each string has the same frequency of each letter of the alphabet.
// Time Complexity: O(n) where n is the number of characters in the passed in strings. We must count the frequency of each letter of the alphabet by traversing the entire string.
// Space Complexity: O(1) No matter the size of the passed in strings we just need the fixed length frequency array to count occurrences. We traverse the arrays in place.
var isAnagram = function (s, t) {
  if (s.length !== t.length) return false;
  ...
  return freq.every((letter) => letter === 0);
};

// #endregion

// #region 28 -- June 25 2025
/**
 * @param {string} s
 * @param {string} t
 * @return {boolean}
...
// Time Complexity: O(n) where n is the number of characters in one of the given strings. We must traverse both strings at the same time all the way through in order to count the frequency of each letter of the alphabet.
// Space Complexity: O(1) no matter the size of the strings passed in we just need the fixed size frequency array to count the 26 letters of the alphabet.
var isAnagram = function (s, t) {
  if (s.length !== t.length) return false;
