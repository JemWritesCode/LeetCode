...
// Time Complexity: O(n + m) where n is the number of nodes in list1 and m is the number of nodes in list2. We must visit each node to change the next pointer.
// Space Complexity: O(1) We only need the dummy and cur no matter the size of the linked list. We reverse the linked list in place.
...
// Asymptotic Notation
// Time Complexity: O(n +) where n is the number of nodes in list 1 and m is the number of nodes in list 2. We must visit each node to change the next pointer and merge the lists.
// Space Complexity: O(1) we just need the dummy node no matter the size of the lists. We traverse the lists in place and change their next pointers.
var mergeTwoLists = function (list1, list2) {
  // go through both lists at the same time. compare the current node between them and insert the lower one.
  // when either of them becomes null insert the rest of the list from the not null list.
  let dummy = new ListNode();
  let cur = dummy;
  while (list1 && list2) {
    if (list1.val > list2.val) {
...
// Time Complexity: O(n + m) where n is the number of nodes in list1 and m is the number of nodes in list2. We must visit every node once to add it to the new list.
// Space Complexity: O(1) When considering asymptotic notation we generally ignore the input and output and only focus on the auxiliary memory. No matter the size of the lists passed in we only need the dummy and curr variables.
var mergeTwoLists = function (list1, list2) {
  let dummy = new ListNode();
  let curr = dummy;
  while (list1 && list2) {
