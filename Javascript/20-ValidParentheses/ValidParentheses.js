...
var isValid = function (s) {
  ...
  for (let c of s) {
    ...
    if (bracketMap[c]) {
      //c is the opening parentheses, bracketMap[c] gets the value so the closing.
      stack.push(bracketMap[c]); // push the closing parentheses into the stack.
    } else if (stack.pop() !== c) return false; // pop the last one off the stack (tells us what to expect) and see if it matches c. if it doesn't match expectation this isn't a valid string.
  }
